...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1); //selecting max3421
	BYTE write_to_reg[] = {reg + 2, val};
	if (XSpi_Transfer(&SpiInstance, write_to_reg, NULL, 2) != 0) xil_printf("error in regwr");
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE bytes_to_write[nbytes + 1];
	bytes_to_write[0] = reg + 2;
	memcpy(bytes_to_write + 1, data, nbytes);
	if (XSpi_Transfer(&SpiInstance, bytes_to_write, NULL, nbytes + 1) != 0) xil_printf("error in multiple byte write");
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE send_reg[] = {reg, 0};
	BYTE recv_buf[2];
	if (XSpi_Transfer(&SpiInstance, send_reg, recv_buf, 2) != 0) xil_printf("error in reg read");
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return recv_buf[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	send_buf[0] = reg;
	if (XSpi_Transfer(&SpiInstance, send_buf, recv_buf, nbytes + 1) != 0) xil_printf("error in mult byte read");
...
void MAX3421E_init(void) {
	...
	//start USB timer
	Status = XTmrCtr_Initialize(&Usb_timer, XPAR_TMRCTR_0_DEVICE_ID);
	if (Status != XST_SUCCESS) {
